...
public:
    ...
    void open(const std::string& clientID, const std::string& server) override {
        std::cout << "Opening persistent store for " << clientID << " on " << server << std::endl;
    }
    ...
    void put(const std::string& key, const std::vector<mqtt::string_view>& bufs) override {
        ...
        std::cout << "Adding data '" << key << " : " << holder << "' to key store" << std::endl;
        storage[key] = std::move(holder);
    }

    mqtt::string_view get(const std::string& key) const override {
        std::cout << "searching for '" << key << "' in persistent storage" << std::endl;
...
class user_callback : public virtual mqtt::callback {
    void connection_lost(const std::string& cause) override {
        std::string error; 
        if (cause.empty()) {error = "idk";}
        else { error = cause; }
        std::cout << "Lost connection b/c of: " << error << std::endl;
    }

    void delivery_complete(mqtt::delivery_token_ptr token) override {
        std::cout << "Delivered: " << (token ? token->get_message_id() : -1) << std::endl;
    }
public:
};
...
int main(int argc, char* argv[]){
    ...
    if (argc == 3){
        ...
        std::cout << "Usage: " << argv[0] << " {TOPIC} {MESSAGE}" << std::endl;
        return 1;
    }
    ...
    try {
        ...
        std::cout << "Persistence error: " << ex.what() << ": " << ex.get_reason_code() << std::endl;
        return 1;
    }
